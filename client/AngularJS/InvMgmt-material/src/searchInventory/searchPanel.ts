...
export class SearchPanel {
      ...
      this.httpService.searchContainer(barcode).subscribe( {
        ...
        error: (errorResponse: HttpErrorResponse) => {
          if (errorResponse.status == 404) {
            this.appUtils.showDialog("Barcode is invalid", []);
          }
          else {
            this.appUtils.serverError("Search container failed", errorResponse);
          }
        }
      });
    }
      ...
      this.httpService.updateVial(this.dataForm.value).subscribe( {
        ...
        next: (resp: any) => {
          if (resp.success === true) {
            this.dataForm.reset();
            this.dataForm.patchValue(resp.data);
            this.appUtils.showDialog("Success!",["Update successful!!"]);
          }
          else {
            alert("Unknown success value returned " + resp.success);
          }
        ...
        // like extJS ajax failure
        error: (errorResponse: HttpErrorResponse) => {
          this.appUtils.serverError("Update vial failed", errorResponse);
        }
      });
    }
  }
  ...
  deleteContainer() {
    if (confirm("Are you sure you want to delete the vial??")) {
      this.httpService.deleteVial(this.dataForm.value).subscribe( {
        ...
        next: (resp: any) => {
          if (resp.success === true) {
            this.dataForm.reset();
            // this.dataForm.patchValue(resp.data);
            this.appUtils.showDialog("Success!", ["Vial successfully deleted!!"]);
          }
          else {
            alert("Unknown success value returned " + resp.success);
          }
        ...
        // like extJS ajax failure
        error: (errorResponse: HttpErrorResponse) => {
          this.appUtils.serverError("Delete vial failed", errorResponse);
        }
      });
    }
  }
