import { Injectable } from '@angular/core';
import { HttpClient, HttpHeaders, HttpParams, HttpParamsOptions } from  '@angular/common/http';

@Injectable({
  providedIn: 'root'
})

/*
const request = new Request(
    /+ URL +/,
    {
        method: 'POST',
        headers: {'X-CSRFToken': csrftoken},
        mode: 'same-origin' // Do not send CSRF token to another domain.
    }
);
fetch(request).then(function(response) {
    // ...
});
*/

export class HttpService {

    private baseURL = '/service/invmgmt/';
    private headers = new HttpHeaders({
      "Content-Type": "application/json",
      "Accept": "application/json"
    });
    private mode = 'same-origin';  // Do not send CSRF token to another domain.

    constructor(private http: HttpClient) { 
        // get the csrf token and apply it to the headers
    	let csrftoken:string = this.get_csrftoken();
    	console.log("*** csrf token " + csrftoken);
    	console.log("*** headers " + this.headers.keys().length);
    	if (csrftoken !== null) {
    	    // the set method returns a NEW copy of the headers
    	    this.headers = this.headers.set('X-CSRFToken', csrftoken);
    	    console.log("*** constructor - keys len " + this.headers.keys().length);
    	}
      
    }
    private what: any;
    
    get_csrftoken() : string {
	    let cookies = document.cookie.split(";"),
		csrfTokenValue:any = null;
	    cookies.forEach((item) => {
		let x = item.split("=");
		if (x[0] === "csrftoken") {
		    csrfTokenValue = x[1];
		    // return false;
		}
	    });
	    return csrfTokenValue;
    }

    // just a test
    getContainerTypes() {
      let url = this.baseURL + "container_type";
      return this.http.get(url);
    }

    addVial(vial: any) {
      let url = this.baseURL + "container";
      return this.http.post(url=url, JSON.stringify(vial), {headers: this.headers});
    }

    locateVial(data:any) {
      let url = this.baseURL + "locate_container";
      return this.http.put(url=url, JSON.stringify(data), {headers: this.headers});
    }

    searchContainer(barcode: string) {
      let url = this.baseURL + "container_by_barcode",
          httpParams = new HttpParams().set("barcode", barcode);
        return this.http.get(url, {params:httpParams});
    }

    updateVial(vial: any) {
      let url = this.baseURL + "container/" + vial.id;
      return this.http.put(url=url, JSON.stringify(vial), {headers: this.headers});
    }

    deleteVial(vial: any) {
      let url = this.baseURL + "container/" + vial.id;
      return this.http.delete(url=url, {headers: this.headers});
    }

    pickList(values: [any]) {
      let url = this.baseURL + "pick_list";
      return this.http.post(url=url, JSON.stringify(values), {headers: this.headers});
    }
}
