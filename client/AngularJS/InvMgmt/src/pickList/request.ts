  ...
  doRequest() {
    if (!this.dataForm.valid) {
      ...
      this.dataArray.controls.forEach((fg:FormGroup,idx,arr) => {
        ...
        let errs = getFormErrors(this.fieldNames, fg);
        if (errs.length > 0) {
      ...
      if (values.length) {
        this.httpService.pickList(values).subscribe( {
          ...
          next: (resp: any) => {
            if (resp.success === true) {
              ...
              if (available_samples == null || unavailable_samples == null) {
                console.dir(resp);
                displayAlert("Did not get back lists of available and unavailable samples", null);
              }
              else {
                if (available_samples.length < 1) {
                  displayAlert("No samples were available", null);
                }
                else {
          ...
          error: (errorResponse: HttpErrorResponse) => {
            serverError("Pick list failed", errorResponse);
          }
        });
      }
      else {
        displayAlert("No values were entered !!!", null);
      }
    }
  }
