    ...
    if cntr:
        ...
        while True:
            ...
            if not (parent_bc and parent_pos):
                ...
                break
            parent_cntr = db.get_by_id("container", parent_bc)
            if not parent_cntr:
                break
...
def _mk_container_data(cntr: dict) -> dict:
    ...
    if lot:
        ...
        container_list = []
        child_cntrs = db.run_query("container", {"parent_barcode":cntr['barcode']})
        ...
        cntr_type = db.get_by_id("container-type", d['container_type'])
...
def get_reagents(limit: int = 100) -> list:
    l = db.run_query(collection="reagent", filter={}, options={"limit": limit})
    return l

def get_lots(limit: int = 100) -> list:
    l = db.run_query(collection="lot", filter={}, options={"limit": limit})
    return l

def get_container_types() -> list:
    # return _list_to_json(ContainerType.objects.all())
    return db.run_query(collection="container_type", filter={})
...
def get_container_by_id(id: str) -> dict:
    t1 = datetime.now()
    cntr = db.get_by_id("container", id)
    if cntr is not None:
...
def locate_container(frm: SampleContainerForm) -> dict:
    ...
    if not frm.is_valid():
        raise ApplicationException("Form data is not valid!!!!")
    data = frm.clean()
    cntr = db.get_by_id("container", data['barcode'])
    if cntr is None:
    ...
    cntr_type = db.get_by_id("container-type", "rack")
    if cntr_type is None:
    ...
    # note that an empty list will bring back all fields and the field name can even be one which
    # does not exist, at which point we get the id
    cntr_list = db.run_query("container", fields=["_"],
                             filter={"parent_barcode": parent_bc, "parent_position": parent_pos})

    if cntr_list:
        raise ApplicationException("Position is already occupied.")
    cntr['parent_barcode'] = parent_bc
    cntr['parent_position'] = parent_pos
    db.update("container", cntr)
    return _mk_container_data(cntr)
...
def generate_pick_list(form_list: [PickListForm]) -> dict:
    ...
    lots = db.run_query("lot", fields=["upper_reagent"], filter={"upper_reagent": {"$in": reagent_list}})
    for l in lots:
        x = reagentLots[l['upper_reagent']]['lots']
        x.append(l['name'])
    ...
    for rec in data_list:
        ...
        # search for containers with any of the reagent lots where the concentration matches and the amount
        # is >= the requested amount.  sort ny amount so we get the container with an amount closest to that
        # requested
        filter = { "amount": {"$gte": amount}, "concentration": concentration, "lot": {"$in": lots} }
        options = { "sort": {"amount": 1} }
        result = db.run_query("container", filter=filter, options=options)
        if result:
