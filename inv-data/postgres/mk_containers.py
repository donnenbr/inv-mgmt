...
CONTAINER_INSERT_SQL = "insert into container(barcode,type_id) values (%s, %s)"
CONT_CONT_INSERT_SQL = "insert into container_container (container_id,parent_container_id,position) values (%s, %s, %s)"
...
print("shelves ...")
barcode = SHELF_BC
cur.execute("select id from container where type_id = %s", (freezer_type_id,))
shelf_data = []
for row in cur.fetchall():
...
cur.executemany(CONTAINER_INSERT_SQL, ins_data)
ins_data.clear()
cur.execute("select id,barcode from container where type_id = %s", (shelf_type_id,))
for row in cur.fetchall():
...
rack_data = []
barcode = RACK_BC
cur.execute("select id from container where type_id = %s", (shelf_type_id,))
for row in cur.fetchall():
...
cur.executemany(CONTAINER_INSERT_SQL, ins_data)
ins_data.clear()
cur.execute("select id,barcode from container where type_id = %s", (rack_type_id,))
for row in cur.fetchall():
	container_d[row[1]] = row[0]
for row in rack_data:
	barcode,parent_id,position = row
