...
CONT_CONT_UPDATE_SQL = "update container_container set container_id=:container_id where id=:location_id"
...
cur.execute("""update container_container set container_id = null where parent_container_id in (
	select id from container where type_id=:1) and container_id is not null""",(vial_type_id,))
cur.execute("delete from container where type_id = :1", (vial_type_id,))
conn.commit()
...
vial_ids = []
cur.execute("select id from container where type_id=:cont_type_id", {"cont_type_id":vial_type_id})
while True:
	rows = cur.fetchmany()
	if not rows:
		break
